...
import { isActiveStatus, isFailedStatus, isFinalStatus, normalizeTaskStatus } from "./status.js";
...
export async function buildTeamSnapshot(ctx, options = {}) {
  ...
  const subagents = attachSubagentDispatcherMetadata(
    attachParentSessionTitles(
      mergeSubagentRuns(visibleTaskSubagents, visibleRunRecords),
      sessionTitleByPath,
    ),
    sessionOwnerByPath,
    agents,
...
function buildSessionTitleMap(sessionResults) {
  const map = new Map();
...
function buildSessionOwnerMap(sessionResults, agents = []) {
...
function attachParentSessionTitles(subagents, sessionTitleByPath) {
...
function attachSubagentDispatcherMetadata(subagents, sessionOwnerByPath, agents = []) {
  const agentById = new Map((agents || []).map((agent) => [agent.id, agent]));
  return subagents.map((task) => {
...
function inferAgentIdFromSessionPath(sessionPath) {
...
function sessionTitleFileCandidates(filePath) {
  if (!filePath) return [];
  if (isArchivedSessionPath(filePath)) return [filePath];
  return [filePath, path.join(path.dirname(filePath), "archived", path.basename(filePath))];
}
