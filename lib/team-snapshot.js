...
export async function buildTeamSnapshot(ctx, options = {}) {
  ...
  const activeParentSessionPaths = new Set(
    sessionResults.flatMap(([, sessions]) => sessions.map((session) => pathKey(session.path)).filter(Boolean)),
  );

  const normalizedTasks = normalizeTasks(tasks);
  const subagents = mergeSubagentRuns(
    normalizedTasks.filter((task) => task.type === "subagent"),
    readSubagentRunRecords(ctx).filter((run) => isVisibleSubagentRun(run, activeParentSessionPaths)),
  );
...
function isVisibleSubagentRun(run, activeParentSessionPaths) {
  const parent = run?.parentSessionPath;
  if (!parent) return false;
  return activeParentSessionPaths.has(pathKey(parent));
}
